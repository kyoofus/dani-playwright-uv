...
async def main():
    """메인 실행 함수"""
    print("=== 네이버 부동산 크롤러 ===")
    print("이 프로그램은 네이버 부동산에서 부동산 정보를 수집합니다.")
    print()
    
    # 사용자 입력 받기
    try:
        center_lat = float(input("중심 위도를 입력하세요 (예: 37.3642443): ") or "37.3642443")
        center_lon = float(input("중심 경도를 입력하세요 (예: 127.1084674): ") or "127.1084674")
        radius = float(input("검색 반경을 입력하세요 (도 단위, 예: 0.005): ") or "0.005")
    except ValueError:
    ...
    try:
        print(f"\n크롤링 시작: 중심좌표 ({center_lat}, {center_lon}), 반경 {radius}")
